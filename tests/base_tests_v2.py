...
def test_torchcast_virtual_unscaled_base(
    ...
    tcast.initialize(roundmode=roundmode, computemode=computemode, castmode=castmode)
    ...
    if tcast.Modes.cast != tcast.CastMode.VIRTUAL:
        tensor_tc = tensor_tc.output
...
def test_torchcast_virtual_unscaled_torch_base(
    ...
    tcast.initialize(roundmode=roundmode, computemode=computemode, castmode=castmode)
    tensor = torch.randn(shape, device="cuda", dtype=torch_dtype)
    tensor_pt = tensor.to(dtype.nspec.torch_dtype)
    tensor_tc = tcast.cast(tensor, dtype)
    if tcast.Modes.cast != tcast.CastMode.VIRTUAL:
        tensor_tc = tensor_tc.output
