...
def test_mx_scaled_datatypes(datatype, roundmode):
    ...
    mx_specs["round"] = roundmode
    tensor_mx = quantize_mx_op(tensor, mx_specs, elem_format=datatype, axes=-1, round=roundmode).float()
    if "fp" in datatype:
        datatype = datatype[4:]
    if "e4" in datatype:
        datatype += "fn"
    ...
    tensor_tcast = tcast.vcast(tensor, dtype=tcast_dt, roundmode=roundmode, scalemode="max").float()
    compare_2(tensor_mx, tensor_tcast)
